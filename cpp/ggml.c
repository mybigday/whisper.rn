...
static BOOL CALLBACK wsp_ggml_time_init_once(PINIT_ONCE once, PVOID param, PVOID *ctx) {
    UNUSED(once);
    UNUSED(param);
    UNUSED(ctx);
...
void wsp_ggml_time_init(void) {
    ...
    InitOnceExecuteOnce(&once, wsp_ggml_time_init_once, NULL, NULL);
}
...
FILE * wsp_ggml_fopen(const char * fname, const char * mode) {
    ...
    if (wfname) {
        // convert mode (UTF-8)
        wchar_t * wmode = wsp_ggml_mbstowcs(mode);
        if (wmode) {
            // open file
...
static const struct wsp_ggml_type_traits type_traits[WSP_GGML_TYPE_COUNT] = {
    ...
    [WSP_GGML_TYPE_Q1_0] = {
        ...
        .is_quantized             = true,
        .to_float                 = (wsp_ggml_to_float_t) wsp_dewsp_quantize_row_q1_0,
        .from_float_ref           = (wsp_ggml_from_float_t) wsp_quantize_row_q1_0_ref,
    },
    [WSP_GGML_TYPE_Q2_0] = {
        .type_name                = "q2_0",
        .blck_size                = QK2_0,
        ...
        .is_quantized             = true,
        .to_float                 = (wsp_ggml_to_float_t) wsp_dewsp_quantize_row_q2_0,
        .from_float_ref           = (wsp_ggml_from_float_t) wsp_quantize_row_q2_0_ref,
    },
    [WSP_GGML_TYPE_Q4_0] = {
        .type_name                = "q4_0",
        .blck_size                = QK4_0,
...
static_assert(WSP_GGML_OP_COUNT == 101, "WSP_GGML_OP_COUNT != 101");
...
enum wsp_ggml_type wsp_ggml_ftype_to_wsp_ggml_type(enum wsp_ggml_ftype ftype) {
    ...
    switch (ftype) {
        ...
        case WSP_GGML_FTYPE_MOSTLY_Q1_0:          wtype = WSP_GGML_TYPE_Q1_0;  break;
        case WSP_GGML_FTYPE_MOSTLY_Q2_0:          wtype = WSP_GGML_TYPE_Q2_0;  break;
...
static bool wsp_ggml_is_contiguous_m_n(const struct wsp_ggml_tensor * tensor, int m, int n) {
...
bool wsp_ggml_is_contiguous_0(const struct wsp_ggml_tensor * tensor) {
    return wsp_ggml_is_contiguous_m_n(tensor, 0, WSP_GGML_MAX_DIMS);
}

bool wsp_ggml_is_contiguous_1(const struct wsp_ggml_tensor * tensor) {
    return wsp_ggml_is_contiguous_m_n(tensor, 1, WSP_GGML_MAX_DIMS);
}

bool wsp_ggml_is_contiguous_2(const struct wsp_ggml_tensor * tensor) {
    return wsp_ggml_is_contiguous_m_n(tensor, 2, WSP_GGML_MAX_DIMS);
}

bool wsp_ggml_is_contiguous_to_1(const struct wsp_ggml_tensor * tensor) {
    return wsp_ggml_is_contiguous_m_n(tensor, 0, 1);
}

bool wsp_ggml_is_contiguous_to_2(const struct wsp_ggml_tensor * tensor) {
    return wsp_ggml_is_contiguous_m_n(tensor, 0, 2);
}

bool wsp_ggml_is_contiguous_to_3(const struct wsp_ggml_tensor * tensor) {
    return wsp_ggml_is_contiguous_m_n(tensor, 0, 3);
}
...
void wsp_ggml_mul_mat_set_hint(
        struct wsp_ggml_tensor * a,
    ...
    wsp_ggml_set_op_params_i32(a, 1, hint_i32);
}
...
struct wsp_ggml_tensor * wsp_ggml_set_rows(
    ...
    WSP_GGML_ASSERT(b->type == WSP_GGML_TYPE_F32 || b->type == WSP_GGML_TYPE_F16);
...
static struct wsp_ggml_tensor * wsp_ggml_rope_impl(
    ...
    params[15] = 0; // n_offs, set via wsp_ggml_rope_set_offset()
...
struct wsp_ggml_tensor * wsp_ggml_rope_set_offset(
        struct wsp_ggml_tensor  * a,
        int                   n_offs) {
    WSP_GGML_ASSERT(a->op == WSP_GGML_OP_ROPE || a->op == WSP_GGML_OP_ROPE_BACK);
    WSP_GGML_ASSERT(n_offs >= 0);

    const int32_t mode = wsp_ggml_get_op_params_i32(a, 2);
    WSP_GGML_ASSERT(mode != WSP_GGML_ROPE_TYPE_VISION);
...
struct wsp_ggml_tensor * wsp_ggml_conv_1d(
        ...
        int                   p0,
        int                   d0) {
    struct wsp_ggml_tensor * im2col = wsp_ggml_im2col(ctx, a, b, s0, 0, p0, 0, d0, 0, false, a->type == WSP_GGML_TYPE_BF16 ? WSP_GGML_TYPE_F32 : WSP_GGML_TYPE_F16); // [N, OL, IC * K]
...
struct wsp_ggml_tensor * wsp_ggml_conv_1d_dw(
    ...
    struct wsp_ggml_tensor * im2col = wsp_ggml_im2col(ctx, a, new_b, s0, 0, p0, 0, d0, 0, false, a->type == WSP_GGML_TYPE_BF16 ? WSP_GGML_TYPE_F32 : WSP_GGML_TYPE_F16);
...
struct wsp_ggml_tensor * wsp_ggml_conv_2d(
        ...
        int                   d0,
        int                   d1) {
    struct wsp_ggml_tensor * im2col = wsp_ggml_im2col(ctx, a, b, s0, s1, p0, p1, d0, d1, true, a->type == WSP_GGML_TYPE_BF16 ? WSP_GGML_TYPE_F32 : WSP_GGML_TYPE_F16); // [N, OH, OW, IC * KH * KW]
...
struct wsp_ggml_tensor * wsp_ggml_conv_3d(
    ...
    struct wsp_ggml_tensor * im2col = wsp_ggml_im2col_3d(ctx, a, b, IC, s0, s1, s2, p0, p1, p2, d0, d1, d2, a->type == WSP_GGML_TYPE_BF16 ? WSP_GGML_TYPE_F32 : WSP_GGML_TYPE_F16); // [N*OD, OH, OW, IC * KD * KH * KW]
...
struct wsp_ggml_tensor * wsp_ggml_conv_2d_dw(
    ...
    struct wsp_ggml_tensor * im2col = wsp_ggml_im2col(ctx, new_a,
                                        ...
                                        s0, s1, p0, p1, d0, d1, true, a->type == WSP_GGML_TYPE_BF16 ? WSP_GGML_TYPE_F32 : WSP_GGML_TYPE_F16); // [N * IC, OH, OW, KH * KW]
...
struct wsp_ggml_tensor * wsp_ggml_ssm_scan(
    ...
    struct wsp_ggml_tensor * result = wsp_ggml_new_tensor_1d(ctx, WSP_GGML_TYPE_F32, wsp_ggml_nelements(x) + K*s->ne[0]*s->ne[1]*s->ne[2]*ids->ne[0]);
...
// wsp_ggml_lightning_indexer

struct wsp_ggml_tensor * wsp_ggml_lightning_indexer(
        struct wsp_ggml_context * ctx,
        struct wsp_ggml_tensor  * q,
        struct wsp_ggml_tensor  * k,
    ...
    WSP_GGML_ASSERT(    mask->type == WSP_GGML_TYPE_F16);
    WSP_GGML_ASSERT(      q->ne[0] == k->ne[0]);
    WSP_GGML_ASSERT(   mask->ne[0] == k->ne[2]);
    WSP_GGML_ASSERT(      q->ne[1] == weights->ne[0]);
    WSP_GGML_ASSERT(      k->ne[1] == 1);
    WSP_GGML_ASSERT(   mask->ne[1] == q->ne[2]);
    WSP_GGML_ASSERT(      q->ne[2] == weights->ne[1]);
    WSP_GGML_ASSERT(weights->ne[2] == 1);
    WSP_GGML_ASSERT(   mask->ne[2] == 1);
    WSP_GGML_ASSERT(      q->ne[3] == k->ne[3]);
    WSP_GGML_ASSERT(      k->ne[3] == weights->ne[3]);
    WSP_GGML_ASSERT(weights->ne[3] % mask->ne[3] == 0);

    int64_t ne[4] = { k->ne[2], q->ne[2], 1, q->ne[3] };
    struct wsp_ggml_tensor * result = wsp_ggml_new_tensor(ctx, WSP_GGML_TYPE_F32, 4, ne);

    result->op   = WSP_GGML_OP_LIGHTNING_INDEXER;
    result->src[0] = q;
    result->src[1] = k;
...
// wsp_ggml_dsv4_hc_comb

struct wsp_ggml_tensor * wsp_ggml_dsv4_hc_comb(
        struct wsp_ggml_context * ctx,
    ...
    WSP_GGML_ASSERT(hc > 0);
    WSP_GGML_ASSERT(hc == 4);
    WSP_GGML_ASSERT(mixes->ne[2] == 1);
    WSP_GGML_ASSERT(mixes->ne[3] == 1);
    WSP_GGML_ASSERT(scale->ne[0] >= 3);
    WSP_GGML_ASSERT(scale->ne[1] == 1);
    WSP_GGML_ASSERT(scale->ne[2] == 1);
    WSP_GGML_ASSERT(scale->ne[3] == 1);
    WSP_GGML_ASSERT(base->ne[0] == hc_mix_dim);
    WSP_GGML_ASSERT(base->ne[1] == 1);
    WSP_GGML_ASSERT(base->ne[2] == 1);
    WSP_GGML_ASSERT(base->ne[3] == 1);

    struct wsp_ggml_tensor * result = wsp_ggml_new_tensor_3d(ctx, WSP_GGML_TYPE_F32, hc, hc, n_tokens);

    wsp_ggml_set_op_params_f32(result, 0, eps);
    wsp_ggml_set_op_params_i32(result, 1, n_iter);

    result->op     = WSP_GGML_OP_DSV4_HC_COMB;
    result->src[0] = mixes;
    result->src[1] = scale;
    result->src[2] = base;
...
// wsp_ggml_dsv4_hc_pre

struct wsp_ggml_tensor * wsp_ggml_dsv4_hc_pre(
        struct wsp_ggml_context * ctx,
        struct wsp_ggml_tensor  * x,
        struct wsp_ggml_tensor  * weights) {
    ...
    WSP_GGML_ASSERT(hc > 0);
    WSP_GGML_ASSERT(x->ne[3] == 1);
    WSP_GGML_ASSERT(weights->ne[0] == hc);
    WSP_GGML_ASSERT(weights->ne[1] == n_tokens);
    WSP_GGML_ASSERT(weights->ne[2] == 1);
    WSP_GGML_ASSERT(weights->ne[3] == 1);

    struct wsp_ggml_tensor * result = wsp_ggml_new_tensor_2d(ctx, WSP_GGML_TYPE_F32, n_embd, n_tokens);

    result->op     = WSP_GGML_OP_DSV4_HC_PRE;
    result->src[0] = x;
...
// wsp_ggml_dsv4_hc_post

struct wsp_ggml_tensor * wsp_ggml_dsv4_hc_post(
        struct wsp_ggml_context * ctx,
        struct wsp_ggml_tensor  * x,
    ...
    WSP_GGML_ASSERT(hc > 0);
    WSP_GGML_ASSERT(x->ne[2] == 1);
    WSP_GGML_ASSERT(x->ne[3] == 1);

    WSP_GGML_ASSERT(residual->ne[0] == n_embd);
    WSP_GGML_ASSERT(residual->ne[2] == n_tokens);
    WSP_GGML_ASSERT(residual->ne[3] == 1);

    WSP_GGML_ASSERT(post->ne[0] == hc);
    WSP_GGML_ASSERT(post->ne[1] == n_tokens);
    WSP_GGML_ASSERT(post->ne[2] == 1);
    WSP_GGML_ASSERT(post->ne[3] == 1);

    WSP_GGML_ASSERT(comb->ne[0] == hc);
    WSP_GGML_ASSERT(comb->ne[1] == hc);
    WSP_GGML_ASSERT(comb->ne[2] == n_tokens);
    WSP_GGML_ASSERT(comb->ne[3] == 1);

    struct wsp_ggml_tensor * result = wsp_ggml_new_tensor_3d(ctx, WSP_GGML_TYPE_F32, n_embd, hc, n_tokens);

    result->op     = WSP_GGML_OP_DSV4_HC_POST;
    result->src[0] = x;
...
void wsp_ggml_build_forward_order(struct wsp_ggml_cgraph * cgraph, struct wsp_ggml_tensor * tensor) {
    wsp_ggml_build_forward_impl(cgraph, tensor, true, false);
}
...
static bool wsp_ggml_is_constant(const struct wsp_ggml_tensor * tensor) {
    return tensor->buffer != NULL && wsp_ggml_backend_buffer_get_usage(tensor->buffer) == WSP_GGML_BACKEND_BUFFER_USAGE_WEIGHTS && (tensor->flags & WSP_GGML_TENSOR_FLAG_PARAM) == 0;
}
...
bool wsp_ggml_can_fuse_subgraph_ext(const struct wsp_ggml_cgraph * cgraph,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        while (view_src) {
            if (wsp_ggml_node_list_find_tensor(cgraph, node_idxs, count, view_src) == -1 && !wsp_ggml_is_constant(view_src)) {
                return false;
            }
            view_src = view_src->view_src;
        }
...
void wsp_ggml_set_output(struct wsp_ggml_tensor * tensor) {
    for (struct wsp_ggml_tensor * cur = tensor; cur != NULL; cur = cur->view_src) {
        cur->flags |= WSP_GGML_TENSOR_FLAG_OUTPUT;
    }
}
...
size_t wsp_ggml_wsp_quantize_chunk(
    ...
    switch (type) {
        case WSP_GGML_TYPE_Q1_0:    result = wsp_quantize_q1_0   (src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
        case WSP_GGML_TYPE_Q2_0:    result = wsp_quantize_q2_0   (src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
