...
#define WSP_GGML_F32Cx8x2_LOAD(x, y)     _mm512_cvtph_ps(_mm256_set_m128i(_mm_loadu_si128((const __m128i *)(y)), _mm_loadu_si128((const __m128i *)(x))))
#define WSP_GGML_F32Cx16_REPEAT_LOAD(x)  _mm512_cvtph_ps(_mm256_set_m128i(x, x))
#endif
// the  _mm256_cvt intrinsics require F16C
#define WSP_GGML_F32Cx8_LOAD(x)     _mm256_cvtph_ps(_mm_loadu_si128((const __m128i *)(x)))
#define WSP_GGML_F32Cx8_REPEAT_LOAD(x, loadMask)     _mm256_cvtph_ps(_mm_shuffle_epi32(_mm_maskload_epi32((int const*)(x), loadMask), 68))
#define WSP_GGML_F32Cx8_REARRANGE_LOAD(x, arrangeMask)     _mm256_cvtph_ps(_mm_shuffle_epi8(_mm_loadu_si128((const __m128i *) x), arrangeMask))
#else
#if defined(__AVX512F__)
static inline __m512 __avx512_f32cx8x2_load(wsp_ggml_fp16_t *x, wsp_ggml_fp16_t *y) {
    float tmp[16];
...
static inline __m256 __avx_repeat_f32cx8_load(wsp_ggml_fp16_t *x) {
    float tmp[8];
...
static inline __m256 __avx_rearranged_f32cx8_load(wsp_ggml_fp16_t *x, __m128i arrangeMask) {
    uint16_t tmphalf[8];
    float tmp[8];
...
#define WSP_GGML_F32Cx8_REPEAT_LOAD(x, loadMask)     __avx_repeat_f32cx8_load(x)
#define WSP_GGML_F32Cx8_REARRANGE_LOAD(x, arrangeMask)     __avx_rearranged_f32cx8_load(x, arrangeMask)
#if defined(__AVX512F__)
#define WSP_GGML_F32Cx8x2_LOAD(x, y)     __avx512_f32cx8x2_load(x, y)
#define WSP_GGML_F32Cx16_REPEAT_LOAD(x)  __avx512_repeat_f32cx16_load(x)
#endif
#endif
#endif
...
void wsp_quantize_q8_0_4x4(const float * restrict x, void * restrict vy, int64_t k) {
    assert(QK8_0 == 32);
    assert(k % QK8_0 == 0);
...
void wsp_quantize_q8_0_4x8(const float * restrict x, void * restrict vy, int64_t k) {
    assert(QK8_0 == 32);
    assert(k % QK8_0 == 0);
    const int nb = k / QK8_0;
    ...
    for (int i = 0; i < nb; i++) {
        for (int row_iter = 0; row_iter < 4; row_iter++) {
            ...
            // Divided by 127.f to mirror results in wsp_quantize_row_q8_0
...
void wsp_quantize_mat_q8_0(const float * restrict x, void * restrict vy, int64_t nrow, int64_t n_per_row, int64_t blck_size_interleave) {
    ...
    if (blck_size_interleave == 4) {
        wsp_quantize_q8_0_4x4(x, vy, n_per_row);
    } else if (blck_size_interleave == 8) {
        wsp_quantize_q8_0_4x8(x, vy, n_per_row);
    } else {
        assert(false);
    }
}
...
void wsp_ggml_gemv_q4_0_8x8_q8_0(int n, float * restrict s, size_t bs, const void * restrict vx, const void * restrict vy, int nr, int nc) {
    ...
    if (wsp_ggml_cpu_has_sve() && wsp_ggml_cpu_get_sve_cnt() == QK8_0) {
        const void * b_ptr = vx;
        const void * a_ptr = vy;
        float * res_ptr = s;
    ...
    if (__riscv_vlenb() >= QK4_0) {
        ...
        for (int x = 0; x < nc / ncols_interleaved; x++) {
            ...
            for (int l = 0; l < nb; l++) {
                ...
                const float a_scale = WSP_GGML_FP16_TO_FP32(a_ptr[l].d);
                const float b_scales[8] = {
                    WSP_GGML_FP16_TO_FP32(b_ptr[l].d[0]),
                    WSP_GGML_FP16_TO_FP32(b_ptr[l].d[1]),
                    WSP_GGML_FP16_TO_FP32(b_ptr[l].d[2]),
                    WSP_GGML_FP16_TO_FP32(b_ptr[l].d[3]),
                    WSP_GGML_FP16_TO_FP32(b_ptr[l].d[4]),
                    WSP_GGML_FP16_TO_FP32(b_ptr[l].d[5]),
                    WSP_GGML_FP16_TO_FP32(b_ptr[l].d[6]),
                    WSP_GGML_FP16_TO_FP32(b_ptr[l].d[7])
                };
...
void wsp_ggml_gemm_q4_0_8x8_q8_0(int n, float * restrict s, size_t bs, const void * restrict vx, const void * restrict vy, int nr, int nc) {
    ...
    if (wsp_ggml_cpu_has_sve() && wsp_ggml_cpu_has_matmul_int8() && wsp_ggml_cpu_get_sve_cnt() == QK8_0) {
        const void * b_ptr = vx;
        const void * a_ptr = vy;
        float * res_ptr = s;
        size_t res_stride = bs * sizeof(float);
        ...
        for (; y < anr / 4; y += 4) {
            ...
            for (int64_t x = xstart; x < nc / 8; x++) {
                ...
                for (int64_t b = 0; b < nb; b++) {
                    ...
                    const __m256 col_scale_f32 = WSP_GGML_F32Cx8_LOAD(b_ptr[b].d);
            ...
            for (int x = 0; x < nc / ncols_interleaved; x++) {
                ...
                for (int l = 0; l < nb; l++) {
                    ...
                    const float a_scales[4] = {
                        WSP_GGML_FP16_TO_FP32(a_ptr[l].d[0]),
                        WSP_GGML_FP16_TO_FP32(a_ptr[l].d[1]),
                        WSP_GGML_FP16_TO_FP32(a_ptr[l].d[2]),
                        WSP_GGML_FP16_TO_FP32(a_ptr[l].d[3])
                    };
                    const float b_scales[8] = {
                        WSP_GGML_FP16_TO_FP32(b_ptr[l].d[0]),
                        WSP_GGML_FP16_TO_FP32(b_ptr[l].d[1]),
