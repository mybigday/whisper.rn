...
template <int64_t INTER_SIZE, wsp_ggml_type PARAM_TYPE>
void wsp_ggml_wsp_quantize_mat_t(const float * WSP_GGML_RESTRICT x, void * WSP_GGML_RESTRICT vy, int64_t nrow, int64_t n_per_row);

template <> void wsp_ggml_wsp_quantize_mat_t<4, WSP_GGML_TYPE_Q8_0>(const float * WSP_GGML_RESTRICT x, void * WSP_GGML_RESTRICT vy, int64_t nrow, int64_t n_per_row) {
    assert(nrow == 4);
    UNUSED(nrow);
    wsp_ggml_wsp_quantize_mat_q8_0_4x4(x, vy, n_per_row);
}

template <> void wsp_ggml_wsp_quantize_mat_t<8, WSP_GGML_TYPE_Q8_0>(const float * WSP_GGML_RESTRICT x, void * WSP_GGML_RESTRICT vy, int64_t nrow, int64_t n_per_row) {
    assert(nrow == 4);
    UNUSED(nrow);
...
template <> void wsp_ggml_wsp_quantize_mat_t<8, WSP_GGML_TYPE_Q8_K>(const float * WSP_GGML_RESTRICT x, void * WSP_GGML_RESTRICT vy, int64_t nrow, int64_t n_per_row) {
    assert(nrow == 4);
    UNUSED(nrow);
    wsp_ggml_wsp_quantize_mat_q8_K_4x8(x, vy, n_per_row);
}

#if defined __riscv_zvfh
template <> void wsp_ggml_wsp_quantize_mat_t<1, WSP_GGML_TYPE_Q8_0>(const float * WSP_GGML_RESTRICT x, void * WSP_GGML_RESTRICT vy, int64_t nrow, int64_t n_per_row) {
    assert(nrow == 4);
    UNUSED(nrow);
    wsp_ggml_wsp_quantize_mat_q8_0_4x1(x, vy, n_per_row);
}

template <> void wsp_ggml_wsp_quantize_mat_t<1, WSP_GGML_TYPE_Q8_K>(const float * WSP_GGML_RESTRICT x, void * WSP_GGML_RESTRICT vy, int64_t nrow, int64_t n_per_row) {
    assert(nrow == 4);
    UNUSED(nrow);
    wsp_ggml_wsp_quantize_mat_q8_K_4x1(x, vy, n_per_row);
}
#endif

template <int M, int N>
static void wsp_ggml_gemv_q6_K_NxM_q8_K_generic_impl(int                        n,
        ...
        for (int l = 0; l < nb; l++) {
            for (int k = 0; k < (qk / (2 * blocklen)); k++) {
                ...
                for (int j = 0; j < ncols_interleaved; j++) {
                    ...
                    sumf[j] +=
                        (sumi_l * scale_l + sumi_h * scale_h) * WSP_GGML_CPU_FP16_TO_FP32(b_ptr[l].d[j]) * a_ptr[l].d;
                }
            }
        }
...
template <int M, int N>
static void wsp_ggml_gemm_q6_K_NxM_q8_K_generic_impl(int                        n,
                                                 float * WSP_GGML_RESTRICT      s,
                                                 size_t                     bs,
                                                 const void * WSP_GGML_RESTRICT vx,
                                                 const void * WSP_GGML_RESTRICT vy,
                ...
                for (int k = 0; k < (qk / (2 * blocklen)); k++) {
                    ...
                    for (int m = 0; m < 4; m++) {
                        for (int j = 0; j < ncols_interleaved; j++) {
                            ...
                            sumf[m][j] += (sumi_l * scale_l + sumi_h * scale_h) * WSP_GGML_CPU_FP16_TO_FP32(b_ptr[l].d[j]) *
                                          a_ptr[l].d[m];
                        }
                    }
                }
...
template <int M, int N>
static void wsp_ggml_gemv_q5_K_NxM_q8_K_generic_impl(int                        n,
                                                 float * WSP_GGML_RESTRICT      s,
                                                 size_t                     bs,
                                                 const void * WSP_GGML_RESTRICT vx,
                                                 const void * WSP_GGML_RESTRICT vy,
            ...
            for (int sb = 0; sb < 8; sb++) {
                ...
                for (int j = 0; j < ncols_interleaved; j++) {
                    sum_minf[j] += mins[j] * (a_ptr[l].bsums[sb * 2] + a_ptr[l].bsums[sb * 2 + 1]) *
                                   WSP_GGML_CPU_FP16_TO_FP32(b_ptr[l].dmin[j]) * a_ptr[l].d;
                }
            }
        }
        for (int j = 0; j < ncols_interleaved; j++) {
...
template <int M, int N>
static void wsp_ggml_gemm_q5_K_NxM_q8_K_generic_impl(int                        n,
                                                 float * WSP_GGML_RESTRICT      s,
                                                 size_t                     bs,
                                                 const void * WSP_GGML_RESTRICT vx,
                                                 const void * WSP_GGML_RESTRICT vy,
                    ...
                    for (int m = 0; m < 4; m++) {
                        ...
                        for (int j = 0; j < ncols_interleaved; j++) {
                            sum_minf[m][j] += mins[j] * (bsums[0] + bsums[1]) *
                                              WSP_GGML_CPU_FP16_TO_FP32(b_ptr[l].dmin[j]) * a_ptr[l].d[m];
                        }
                    }
                }
            }
    ...
    for(int x = 0; x < nc / ncols_interleaved; x++) {
        ...
        for (int l = 0; l < nb; l++) {
            ...
            for(int sb = 0; sb < 8; sb++) {
                ...
                for(int j = 0; j < ncols_interleaved; j++){
                    sum_minf[j] += ((mins[j * 2] >> 4) * a_ptr[l].bsums[sb * 2] + (mins[(j * 2)+ 1] >> 4) * a_ptr[l].bsums[sb * 2 + 1]) * WSP_GGML_FP16_TO_FP32(b_ptr[l].dmin[j]) * a_ptr[l].d;
                }
            }
        }
        for (int j = 0; j < ncols_interleaved; j++) {
...
void wsp_ggml_gemv_q5_K_8x4_q8_K_generic(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, const void * WSP_GGML_RESTRICT vy, int nr, int nc) {
    wsp_ggml_gemv_q5_K_NxM_q8_K_generic_impl<4, 8>(n, s, bs, vx, vy, nr, nc);
}
...
void wsp_ggml_gemv_q5_K_8x8_q8_K_generic(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, const void * WSP_GGML_RESTRICT vy, int nr, int nc) {
    wsp_ggml_gemv_q5_K_NxM_q8_K_generic_impl<8, 8>(n, s, bs, vx, vy, nr, nc);
}
...
void wsp_ggml_gemv_q6_K_8x4_q8_K_generic(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, const void * WSP_GGML_RESTRICT vy, int nr, int nc) {
    wsp_ggml_gemv_q6_K_NxM_q8_K_generic_impl<4, 8>(n, s, bs, vx, vy, nr, nc);
}
...
void wsp_ggml_gemv_q6_K_8x8_q8_K_generic(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, const void * WSP_GGML_RESTRICT vy, int nr, int nc) {
    wsp_ggml_gemv_q6_K_NxM_q8_K_generic_impl<8, 8>(n, s, bs, vx, vy, nr, nc);
}
        ...
        for (int l = 0; l < nb; l++) {
            for (int k = 0; k < (qk / (2 * blocklen)); k++) {
                for (int j = 0; j < ncols_interleaved; j++) {
                    ...
                    sumf[j] += sumi * WSP_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[l].e[j]) * WSP_GGML_CPU_FP16_TO_FP32(a_ptr[l].d);
                }
            }
        }
        for (int j = 0; j < ncols_interleaved; j++) s[x * ncols_interleaved + j] = sumf[j];
    ...
    for (int x = 0; x < nc / ncols_interleaved; x++) {
        ...
        for (int l = 0; l < nb; l++) {
            ...
            for (int sb = 0; sb < 8; sb++) {
                ...
                for (int j = 0; j < ncols_interleaved; j++) {
                    sum_minf[j] += min[j] * (a_ptr[l].bsums[sb * 2] + a_ptr[l].bsums[sb * 2 + 1]) * WSP_GGML_CPU_FP16_TO_FP32(b_ptr[l].dmin[j]) * a_ptr[l].d;
                }
            }
    ...
    for (int col_tile = 0; col_tile < nc; col_tile += 16) {
        ...
        for (int k_block = 0; k_block < nb; ++k_block) {
            ...
            for (int col = 0; col < 16; ++col) {
                ...
                float d_rhs = WSP_GGML_FP16_TO_FP32(x_ptr[k_block].d[col]);
                float dm_rhs = WSP_GGML_FP16_TO_FP32(x_ptr[k_block].dmin[col]);
                ...
                for(int sb = 0; sb < 8; sb++) {
                    ...
                    for(int m = 0; m < 4; m++) {
                        ...
                        for(int j = 0; j < ncols_interleaved; j++) {
                            ...
                            sum_minf[m][j] += (mins_prod) * WSP_GGML_FP16_TO_FP32(b_ptr[l].dmin[j]) * a_ptr[l].d[m];
                        }
                    }
                }
            }
...
void wsp_ggml_gemm_q5_K_8x4_q8_K_generic(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, const void * WSP_GGML_RESTRICT vy, int nr, int nc) {
    wsp_ggml_gemm_q5_K_NxM_q8_K_generic_impl<4, 8>(n, s, bs, vx, vy, nr, nc);
}
...
void wsp_ggml_gemm_q5_K_8x8_q8_K_generic(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, const void * WSP_GGML_RESTRICT vy, int nr, int nc) {
    wsp_ggml_gemm_q5_K_NxM_q8_K_generic_impl<8, 8>(n, s, bs, vx, vy, nr, nc);
}
...
void wsp_ggml_gemm_q6_K_8x4_q8_K_generic(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, const void * WSP_GGML_RESTRICT vy, int nr, int nc) {
    wsp_ggml_gemm_q6_K_NxM_q8_K_generic_impl<4, 8>(n, s, bs, vx, vy, nr, nc);
}
...
void wsp_ggml_gemm_q6_K_8x8_q8_K_generic(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, const void * WSP_GGML_RESTRICT vy, int nr, int nc) {
   wsp_ggml_gemm_q6_K_NxM_q8_K_generic_impl<8, 8>(n, s, bs, vx, vy, nr, nc);
}
                ...
                for (int k = 0; k < (qk / (2 * blocklen)); k++) {
                    for (int m = 0; m < 4; m++) {
                        for (int j = 0; j < ncols_interleaved; j++) {
                            ...
                            sumf[m][j] += sumi * WSP_GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[l].e[j]) * WSP_GGML_CPU_FP16_TO_FP32(a_ptr[l].d[m]);
                        }
                    }
                }
            }
            ...
            for (int l = 0; l < nb; l++) {
                ...
                for (int sb = 0; sb < 8; sb++) {
                    ...
                    for(int m = 0; m < 4; m++) {
                        ...
                        for(int j = 0; j < ncols_interleaved; j++) {
                            sum_minf[m][j] += min[j] * bsums * WSP_GGML_CPU_FP16_TO_FP32(b_ptr[l].dmin[j]) * a_ptr[l].d[m];
                        }
                    }
                }
...
static block_q4_Kx8 make_block_q4_Kx8(block_q4_K * in, unsigned int blck_size_interleave) {
    ...
    for (int i = 0; i < 8; i++) {
        out.d[i] = in[i].WSP_GGML_COMMON_AGGR_U.WSP_GGML_COMMON_AGGR_S.d;
    }

    for (int i = 0; i < 8; i++) {
        out.dmin[i] = in[i].WSP_GGML_COMMON_AGGR_U.WSP_GGML_COMMON_AGGR_S.dmin;
    }
...
static int repack_q4_K_to_q4_K_8_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    ...
    WSP_GGML_ASSERT(data_size == nrow * nblocks * sizeof(block_q4_K));
...
static int repack_q4_K_to_q4_K_16_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_Q4_K);
...
static int repack_q2_K_to_q2_K_8_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_Q2_K);
    ...
    WSP_GGML_ASSERT(data_size == nrow * nblocks * sizeof(block_q2_K));
...
static int repack_q2_K_to_q2_K_16_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_Q2_K);
    constexpr int nrows_interleaved = 16;
...
static int repack_q4_0_to_q4_0_16_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_Q4_0);
    constexpr int nrows_interleaved = 16;
...
static int repack_q5_K_to_q5_K_8_bl(struct wsp_ggml_tensor *       t,
                                    int                        interleave_block,
                                    const void * WSP_GGML_RESTRICT data,
                                    size_t                     data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_Q5_K);
    WSP_GGML_ASSERT(interleave_block == 4 || interleave_block == 8);
    constexpr int nrows_interleaved = 8;
    ...
    WSP_GGML_ASSERT(data_size == nrow * nblocks * sizeof(block_q5_K));
...
static int repack_q6_K_to_q6_K_8_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_Q6_K);
    WSP_GGML_ASSERT(interleave_block == 4 || interleave_block == 8);
    constexpr int nrows_interleaved = 8;
    ...
    WSP_GGML_ASSERT(data_size == nrow * nblocks * sizeof(block_q6_K));
...
static int repack_q8_0_to_q8_0_16_bl(struct wsp_ggml_tensor *       t,
                                    int                        interleave_block,
                                    const void * WSP_GGML_RESTRICT data,
                                    size_t                     data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_Q8_0);
...
static int repack_iq4_nl_to_iq4_nl_8_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_IQ4_NL);
    WSP_GGML_ASSERT(interleave_block == 8);
...
static int repack_iq4_nl_to_iq4_nl_16_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_IQ4_NL);
    WSP_GGML_ASSERT(interleave_block == 1);
...
static int repack_mxfp4_to_mxfp4_4_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_MXFP4);
    WSP_GGML_ASSERT(interleave_block == 4);
    ...
    WSP_GGML_ASSERT(data_size == nrow * nblocks * sizeof(block_mxfp4));
...
static int repack_mxfp4_to_mxfp4_8_bl(struct wsp_ggml_tensor * t, int interleave_block, const void * WSP_GGML_RESTRICT data, size_t data_size) {
    WSP_GGML_ASSERT(t->type == WSP_GGML_TYPE_MXFP4);
    WSP_GGML_ASSERT(interleave_block == 8);
...
template <> int repack<block_q4_K, 8, 8>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
...
template <> int repack<block_q5_K, 4, 8>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q5_K_to_q5_K_8_bl(t, 4, data, data_size);
}

template <> int repack<block_q5_K, 8, 8>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q5_K_to_q5_K_8_bl(t, 8, data, data_size);
}

template <> int repack<block_q6_K, 4, 8>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q6_K_to_q6_K_8_bl(t, 4, data, data_size);
}

template <> int repack<block_q6_K, 8, 8>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
...
template <> int repack<block_iq4_nl, 8, 8>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
    return repack_iq4_nl_to_iq4_nl_8_bl(t, 8, data, data_size);
}

template <> int repack<block_mxfp4, 4, 4>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
    return repack_mxfp4_to_mxfp4_4_bl(t, 4, data, data_size);
}

template <> int repack<block_mxfp4, 8, 8>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
...
#if defined __riscv_zvfh
template <> int repack<block_q4_0, 1, 16>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q4_0_to_q4_0_16_bl(t, 1, data, data_size);
}

template <> int repack<block_q4_K, 1, 16>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q4_K_to_q4_K_16_bl(t, 1, data, data_size);
}

template <> int repack<block_iq4_nl, 1, 16>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
    return repack_iq4_nl_to_iq4_nl_16_bl(t, 1, data, data_size);
}

template <> int repack<block_q8_0, 1, 16>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q8_0_to_q8_0_16_bl(t, 1, data, data_size);
}

template <> int repack<block_q2_K, 1, 16>(struct wsp_ggml_tensor * t, const void * data, size_t data_size) {
...
template <> void gemv<block_q4_0, 4, 4, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q4_0_4x4_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q4_0, 8, 4, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q4_0_4x8_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q4_0, 8, 8, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q4_0_8x8_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <>
void gemv<block_q2_K, 8, 8, WSP_GGML_TYPE_Q8_K>(int          n,
                                            float *      s,
                                            size_t       bs,
                                            const void * vx,
                                            const void * vy,
                                            int          nr,
                                            int          nc) {
    wsp_ggml_gemv_q2_K_8x8_q8_K(n, s, bs, vx, vy, nr, nc);
}
...
template <> void gemv<block_q5_K, 4, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q5_K_8x4_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q5_K, 8, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q5_K_8x8_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q6_K, 4, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q6_K_8x4_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q6_K, 8, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q6_K_8x8_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_iq4_nl, 4, 4, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_iq4_nl_4x4_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_iq4_nl, 8, 8, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_iq4_nl_8x8_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_mxfp4, 4, 4, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_mxfp4_4x4_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_mxfp4, 8, 8, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_mxfp4_8x8_q8_0(n, s, bs, vx, vy, nr, nc);
}
...
#if defined __riscv_zvfh
template <> void gemv<block_q4_0, 1, 16, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q4_0_16x1_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q4_K, 1, 16, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q4_K_16x1_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_iq4_nl, 1, 16, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_iq4_nl_16x1_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q8_0, 1, 16, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q8_0_16x1_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q2_K, 1, 16, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemv_q2_K_16x1_q8_K(n, s, bs, vx, vy, nr, nc);
}
#endif
...
template <> void gemm<block_q4_0, 4, 4, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q4_0_4x4_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_q4_0, 8, 4, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q4_0_4x8_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <>
void gemm<block_q4_0, 8, 8, WSP_GGML_TYPE_Q8_0>(int          n,
                                            float *      s,
                                            size_t       bs,
                                            const void * vx,
                                            const void * vy,
...
template <> void gemm<block_q2_K, 8, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q2_K_8x8_q8_K(n, s, bs, vx, vy, nr, nc);
}
...
template <> void gemm<block_q4_K, 8, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q4_K_8x8_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_q5_K, 4, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q5_K_8x4_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_q5_K, 8, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q5_K_8x8_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_q6_K, 4, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q6_K_8x4_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_q6_K, 8, 8, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q6_K_8x8_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_iq4_nl, 4, 4, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_iq4_nl_4x4_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_iq4_nl, 8, 8, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_iq4_nl_8x8_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_mxfp4, 4, 4, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_mxfp4_4x4_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_mxfp4, 8, 8, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_mxfp4_8x8_q8_0(n, s, bs, vx, vy, nr, nc);
}
...
#if defined __riscv_zvfh
template <> void gemm<block_q4_0, 1, 16, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q4_0_16x1_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_q4_K, 1, 16, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q4_K_16x1_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_iq4_nl, 1, 16, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_iq4_nl_16x1_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_q8_0, 1, 16, WSP_GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q8_0_16x1_q8_0(n, s, bs, vx, vy, nr, nc);
}

template <> void gemm<block_q2_K, 1, 16, WSP_GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    wsp_ggml_gemm_q2_K_16x1_q8_K(n, s, bs, vx, vy, nr, nc);
}
#endif
...
template <typename BLOC_TYPE, int64_t INTER_SIZE, int64_t NB_COLS, wsp_ggml_type PARAM_TYPE> class tensor_traits : public tensor_traits_base {
    ...
    bool work_size(int /* n_threads */, const struct wsp_ggml_tensor * op, size_t & size) override {
        ...
        switch (op->op) {
            case WSP_GGML_OP_MUL_MAT:
                {
                    size = wsp_ggml_row_size(PARAM_TYPE, wsp_ggml_nelements(op->src[1]));
                    return true;
                }
            case WSP_GGML_OP_MUL_MAT_ID:
                {
                    size = wsp_ggml_row_size(PARAM_TYPE, wsp_ggml_nelements(op->src[1]));
                    size = WSP_GGML_PAD(size, sizeof(int64_t)); // + padding for next block.
    ...
    void forward_mul_mat_id(wsp_ggml_compute_params * params, wsp_ggml_tensor * op) {
        ...
        WSP_GGML_ASSERT(params->wsize >=
                (WSP_GGML_PAD(nbw3, sizeof(int64_t)) +
...
static const ggml::cpu::tensor_traits * wsp_ggml_repack_get_optimal_repack_type(const struct wsp_ggml_tensor * cur) {
    // instance for Q4
    ...
    // instance for Q5_K
    static const ggml::cpu::repack::tensor_traits<block_q5_K, 4, 8, WSP_GGML_TYPE_Q8_K> q5_K_8x4_q8_K;
    static const ggml::cpu::repack::tensor_traits<block_q5_K, 8, 8, WSP_GGML_TYPE_Q8_K> q5_K_8x8_q8_K;

    // instance for Q6_K
    static const ggml::cpu::repack::tensor_traits<block_q6_K, 4, 8, WSP_GGML_TYPE_Q8_K> q6_K_8x4_q8_K;
    static const ggml::cpu::repack::tensor_traits<block_q6_K, 8, 8, WSP_GGML_TYPE_Q8_K> q6_K_8x8_q8_K;

    // instance for Q2
    static const ggml::cpu::repack::tensor_traits<block_q2_K, 8, 8, WSP_GGML_TYPE_Q8_K> q2_K_8x8_q8_K;
    ...
    // instance for MXFP4
    static const ggml::cpu::repack::tensor_traits<block_mxfp4, 4, 4, WSP_GGML_TYPE_Q8_0> mxfp4_4x4_q8_0;
    static const ggml::cpu::repack::tensor_traits<block_mxfp4, 8, 8, WSP_GGML_TYPE_Q8_0> mxfp4_8x8_q8_0;
    ...
    // an interleave of 1.
#if defined __riscv_zvfh
    static const ggml::cpu::repack::tensor_traits<block_q4_0, 1, 16, WSP_GGML_TYPE_Q8_0> q4_0_16x1_q8_0;
    static const ggml::cpu::repack::tensor_traits<block_q4_K, 1, 16, WSP_GGML_TYPE_Q8_K> q4_K_16x1_q8_K;
    static const ggml::cpu::repack::tensor_traits<block_iq4_nl, 1, 16, WSP_GGML_TYPE_Q8_0> iq4_nl_16x1_q8_0;
    static const ggml::cpu::repack::tensor_traits<block_q8_0, 1, 16, WSP_GGML_TYPE_Q8_0> q8_0_16x1_q8_0;
    static const ggml::cpu::repack::tensor_traits<block_q2_K, 1, 16, WSP_GGML_TYPE_Q8_K> q2_K_16x1_q8_K;
#endif
    ...
    } else if (cur->type == WSP_GGML_TYPE_Q4_K) {
    ...
    } else if (cur->type == WSP_GGML_TYPE_Q5_K) {
        if (wsp_ggml_cpu_has_neon() && wsp_ggml_cpu_has_matmul_int8()) {
            if (cur->ne[1] % 8 == 0) {
    ...
    } else if (cur->type == WSP_GGML_TYPE_Q6_K) {
        if (wsp_ggml_cpu_has_neon() && wsp_ggml_cpu_has_matmul_int8()) {
            if (cur->ne[1] % 8 == 0) {
    ...
    } else if (cur->type == WSP_GGML_TYPE_MXFP4) {
        if (wsp_ggml_cpu_has_avx2()) {
            if (cur->ne[1] % 8 == 0) {
...
static enum wsp_ggml_status wsp_ggml_backend_cpu_repack_buffer_init_tensor(wsp_ggml_backend_buffer_t buffer, struct wsp_ggml_tensor * tensor) {
    tensor->extra = (void *) const_cast<ggml::cpu::tensor_traits *>(wsp_ggml_repack_get_optimal_repack_type(tensor));

    WSP_GGML_UNUSED(buffer);
    return WSP_GGML_STATUS_SUCCESS;
}

static void wsp_ggml_backend_cpu_repack_buffer_set_tensor(wsp_ggml_backend_buffer_t buffer, struct wsp_ggml_tensor * tensor,
...
static const char * wsp_ggml_backend_cpu_repack_buffer_type_get_name(wsp_ggml_backend_buffer_type_t buft) {
    return "CPU_REPACK";

    WSP_GGML_UNUSED(buft);
}

static wsp_ggml_backend_buffer_t wsp_ggml_backend_cpu_repack_buffer_type_alloc_buffer(wsp_ggml_backend_buffer_type_t buft, size_t size) {
    ...
    buffer->buft              = buft;
    buffer->iface.init_tensor = wsp_ggml_backend_cpu_repack_buffer_init_tensor;
    buffer->iface.set_tensor  = wsp_ggml_backend_cpu_repack_buffer_set_tensor;
    buffer->iface.get_tensor  = nullptr;
    buffer->iface.cpy_tensor  = nullptr;
    return buffer;
}

static size_t wsp_ggml_backend_cpu_repack_buffer_type_get_alignment(wsp_ggml_backend_buffer_type_t buft) {
...
class extra_buffer_type : ggml::cpu::extra_buffer_type {
    bool supports_op(wsp_ggml_backend_dev_t, const struct wsp_ggml_tensor * op) override {
        if (    op->op == WSP_GGML_OP_MUL_MAT &&
                ...
                op->src[0]->buffer->buft == wsp_ggml_backend_cpu_repack_buffer_type() &&
                wsp_ggml_repack_get_optimal_repack_type(op->src[0])
                ) {
    ...
    ggml::cpu::tensor_traits * get_tensor_traits(const struct wsp_ggml_tensor * op) override {
        if (op->op == WSP_GGML_OP_MUL_MAT || op->op == WSP_GGML_OP_MUL_MAT_ID) {
            if (op->src[0]->buffer && op->src[0]->buffer->buft == wsp_ggml_backend_cpu_repack_buffer_type()) {
...
wsp_ggml_backend_buffer_type_t wsp_ggml_backend_cpu_repack_buffer_type(void) {
    static struct wsp_ggml_backend_buffer_type wsp_ggml_backend_cpu_buffer_type_repack = {
        /* .iface    = */ {
                           /* .get_name         = */ wsp_ggml_backend_cpu_repack_buffer_type_get_name,
                           /* .alloc_buffer     = */ wsp_ggml_backend_cpu_repack_buffer_type_alloc_buffer,
                           /* .get_alignment    = */ wsp_ggml_backend_cpu_repack_buffer_type_get_alignment,
    ...
    return &wsp_ggml_backend_cpu_buffer_type_repack;
}
