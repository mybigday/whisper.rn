...
// defined in ggml-cpu.c, initialized in wsp_ggml_cpu_init()
extern float wsp_ggml_table_f32_ue4m3[1 << 8];
...
#define WSP_GGML_CPU_UE4M3_TO_FP32(x) wsp_ggml_table_f32_ue4m3[(uint8_t)(x)]
#else
#define WSP_GGML_CPU_UE4M3_TO_FP32(x) wsp_ggml_ue4m3_to_fp32(x)
#endif
...
static inline void __lsx_f16x4_store(wsp_ggml_fp16_t * x, __m128 y) {
    ...
    memcpy(x, &a, sizeof(wsp_ggml_fp16_t) * 4);
}
