        ...
        for (int i = 0; i < nb; ++i, ++x0, ++x1, ++y0, ++y1) {
            ...
            // adjust bias, apply superblock scale
            {
                ...
                const float32x4_t dmins = {
                    WSP_GGML_FP16_TO_FP32(x0->dmin) * y0->d,
                    WSP_GGML_FP16_TO_FP32(x0->dmin) * y1->d,
                    WSP_GGML_FP16_TO_FP32(x1->dmin) * y0->d,
                    WSP_GGML_FP16_TO_FP32(x1->dmin) * y1->d,
                };
                vfsum = vmlsq_f32(vfsum, vcvtq_f32_s32(vld1q_s32(bias)), dmins);

                const float32x4_t superblock_scale = {
                    WSP_GGML_FP16_TO_FP32(x0->d) * y0->d,
                    WSP_GGML_FP16_TO_FP32(x0->d) * y1->d,
                    WSP_GGML_FP16_TO_FP32(x1->d) * y0->d,
                    WSP_GGML_FP16_TO_FP32(x1->d) * y1->d,
                };
