...
void wsp_quantize_row_q8_0(const float * WSP_GGML_RESTRICT x, void * WSP_GGML_RESTRICT vy, int64_t k) {
    assert(QK8_0 == 32);
    assert(k % QK8_0 == 0);
    const int nb = k / QK8_0;

    block_q8_0 * WSP_GGML_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        y[i].d = WSP_GGML_CPU_FP32_TO_FP16(d);
...
void wsp_quantize_row_q8_1(const float * WSP_GGML_RESTRICT x, void * WSP_GGML_RESTRICT vy, int64_t k) {
    assert(k % QK8_1 == 0);
    const int nb = k / QK8_1;

    block_q8_1 * WSP_GGML_RESTRICT y = vy;
#if defined(__ARM_NEON)
    for (int i = 0; i < nb; i++) {
        float32x4_t srcv [8];
        float32x4_t asrcv[8];
        ...
        y[i].s = WSP_GGML_CPU_FP32_TO_FP16(d * vaddvq_s32(accv));
    }
#else
    WSP_GGML_UNUSED(nb);
    // scalar
...
void wsp_ggml_vec_dot_q4_0_q8_0(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
...
#if defined(__ARM_FEATURE_MATMUL_INT8)
    if (nrc == 2) {
        const block_q4_0 * WSP_GGML_RESTRICT vx0 = vx;
        const block_q4_0 * WSP_GGML_RESTRICT vx1 = (const block_q4_0 *) ((const uint8_t*)vx + bx);
        const block_q8_0 * WSP_GGML_RESTRICT vy0 = vy;
        const block_q8_0 * WSP_GGML_RESTRICT vy1 = (const block_q8_0 *) ((const uint8_t*)vy + by);

        float32x4_t sumv0 = vdupq_n_f32(0.0f);

        for (int i = 0; i < nb; i++) {
            const block_q4_0 * WSP_GGML_RESTRICT b_x0 = &vx0[i];
            const block_q4_0 * WSP_GGML_RESTRICT b_x1 = &vx1[i];
            const block_q8_0 * WSP_GGML_RESTRICT b_y0 = &vy0[i];
            const block_q8_0 * WSP_GGML_RESTRICT b_y1 = &vy1[i];
            ...
            float32_t _scale[4] = {
                WSP_GGML_CPU_FP16_TO_FP32(b_x0->d)*WSP_GGML_CPU_FP16_TO_FP32(b_y0->d),
                WSP_GGML_CPU_FP16_TO_FP32(b_x0->d)*WSP_GGML_CPU_FP16_TO_FP32(b_y1->d),
                WSP_GGML_CPU_FP16_TO_FP32(b_x1->d)*WSP_GGML_CPU_FP16_TO_FP32(b_y0->d),
                WSP_GGML_CPU_FP16_TO_FP32(b_x1->d)*WSP_GGML_CPU_FP16_TO_FP32(b_y1->d)
            };
        ...
        case 128:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    const block_q4_0 * WSP_GGML_RESTRICT x0 = &x[ib + 0];
                    const block_q4_0 * WSP_GGML_RESTRICT x1 = &x[ib + 1];
                    const block_q8_0 * WSP_GGML_RESTRICT y0 = &y[ib + 0];
                    const block_q8_0 * WSP_GGML_RESTRICT y1 = &y[ib + 1];
                    ...
                    sumv0 = svmla_n_f32_x(ph4, sumv0, svcvt_f32_s32_x(ph4, svadd_x(ph4,
                                    ...
                                    svdot_s32(svdup_n_s32(0), qx0hs, qy0h))), WSP_GGML_CPU_FP16_TO_FP32(x0->d)*WSP_GGML_CPU_FP16_TO_FP32(y0->d));
                    sumv1 = svmla_n_f32_x(ph4, sumv1, svcvt_f32_s32_x(ph4, svadd_x(ph4,
                                    svdot_s32(svdup_n_s32(0), qx1ls, qy1l),
                                    svdot_s32(svdup_n_s32(0), qx1hs, qy1h))), WSP_GGML_CPU_FP16_TO_FP32(x1->d)*WSP_GGML_CPU_FP16_TO_FP32(y1->d));
                }
        ...
        case 256:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    ...
                    sumv0 = svmla_n_f32_x(svptrue_b32(), sumv0, svcvt_f32_s32_x(svptrue_b32(),
                                svdot_s32(svdup_n_s32(0), qx0s, qy0)), WSP_GGML_CPU_FP16_TO_FP32(x0->d)*WSP_GGML_CPU_FP16_TO_FP32(y0->d));
                    sumv1 = svmla_n_f32_x(svptrue_b32(), sumv1, svcvt_f32_s32_x(svptrue_b32(),
                                svdot_s32(svdup_n_s32(0), qx1s, qy1)), WSP_GGML_CPU_FP16_TO_FP32(x1->d)*WSP_GGML_CPU_FP16_TO_FP32(y1->d));
                }
    ...
    for (; ib + 1 < nb; ib += 2) {
        ...
        sumv0 = vmlaq_n_f32(sumv0, vcvtq_f32_s32(p_0), WSP_GGML_CPU_FP16_TO_FP32(x0->d)*WSP_GGML_CPU_FP16_TO_FP32(y0->d));
        sumv1 = vmlaq_n_f32(sumv1, vcvtq_f32_s32(p_1), WSP_GGML_CPU_FP16_TO_FP32(x1->d)*WSP_GGML_CPU_FP16_TO_FP32(y1->d));
    }
...
void wsp_ggml_vec_dot_q4_1_q8_1(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
...
#if defined(__ARM_FEATURE_MATMUL_INT8)
    if (nrc == 2) {
        const block_q4_1 * WSP_GGML_RESTRICT vx0 = vx;
        const block_q4_1 * WSP_GGML_RESTRICT vx1 = (const block_q4_1 *) ((const uint8_t*)vx + bx);
        const block_q8_1 * WSP_GGML_RESTRICT vy0 = vy;
        const block_q8_1 * WSP_GGML_RESTRICT vy1 = (const block_q8_1 *) ((const uint8_t*)vy + by);

        float32x4_t sumv0 = vdupq_n_f32(0.0f);
        float32x4_t summs0 = vdupq_n_f32(0.0f);

        for (int i = 0; i < nb; i++) {
            const block_q4_1 * WSP_GGML_RESTRICT b_x0 = &vx0[i];
            const block_q4_1 * WSP_GGML_RESTRICT b_x1 = &vx1[i];
            const block_q8_1 * WSP_GGML_RESTRICT b_y0 = &vy0[i];
            const block_q8_1 * WSP_GGML_RESTRICT b_y1 = &vy1[i];

            float32_t summs_t[4] = {
                WSP_GGML_CPU_FP16_TO_FP32(b_x0->m) * WSP_GGML_CPU_FP16_TO_FP32(b_y0->s),
                WSP_GGML_CPU_FP16_TO_FP32(b_x1->m) * WSP_GGML_CPU_FP16_TO_FP32(b_y0->s),
                WSP_GGML_CPU_FP16_TO_FP32(b_x0->m) * WSP_GGML_CPU_FP16_TO_FP32(b_y1->s),
                WSP_GGML_CPU_FP16_TO_FP32(b_x1->m) * WSP_GGML_CPU_FP16_TO_FP32(b_y1->s)
            };
    ...
    for (; ib + 1 < nb; ib += 2) {
        const block_q4_1 * WSP_GGML_RESTRICT x0 = &x[ib + 0];
        const block_q4_1 * WSP_GGML_RESTRICT x1 = &x[ib + 1];
        const block_q8_1 * WSP_GGML_RESTRICT y0 = &y[ib + 0];
        const block_q8_1 * WSP_GGML_RESTRICT y1 = &y[ib + 1];

        summs += WSP_GGML_CPU_FP16_TO_FP32(x0->m) * WSP_GGML_CPU_FP16_TO_FP32(y0->s) + WSP_GGML_CPU_FP16_TO_FP32(x1->m) * WSP_GGML_CPU_FP16_TO_FP32(y1->s);
...
void wsp_ggml_vec_dot_nvfp4_q8_0(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    UNUSED(by);
    ...
    for (int ib = 0; ib < nb; ++ib) {
        ...
        const int8x16_t q4_lo_0 = wsp_ggml_vqtbl1q_s8(values, vandq_u8  (q4bits_0, m4b));
        const int8x16_t q4_hi_0 = wsp_ggml_vqtbl1q_s8(values, vshrq_n_u8(q4bits_0, 4));
    ...
    for (int ib = 0; ib < nb; ++ib) {
        for (int si = 0; si < 4; ++si) {
            const float d = wsp_ggml_ue4m3_to_fp32(x[ib].d[si]);
            const int q8b = si / 2;
            const int q8o = (si % 2) * QK_NVFP4_SUB;
            const float dy = WSP_GGML_CPU_FP16_TO_FP32(y[2*ib + q8b].d);
...
void wsp_ggml_vec_dot_q5_0_q8_0(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (; ib + 1 < nb; ib += 2) {
        const block_q5_0 * WSP_GGML_RESTRICT x0 = &x[ib];
        const block_q5_0 * WSP_GGML_RESTRICT x1 = &x[ib + 1];
        const block_q8_0 * WSP_GGML_RESTRICT y0 = &y[ib];
        const block_q8_0 * WSP_GGML_RESTRICT y1 = &y[ib + 1];
        ...
        sumv0 = vmlaq_n_f32(sumv0, vcvtq_f32_s32(vaddq_s32(
                        ...
                        wsp_ggml_vdotq_s32(vdupq_n_s32(0), v0_0hf, v1_0h))), WSP_GGML_CPU_FP16_TO_FP32(x0->d)*WSP_GGML_CPU_FP16_TO_FP32(y0->d));
        sumv1 = vmlaq_n_f32(sumv1, vcvtq_f32_s32(vaddq_s32(
                        wsp_ggml_vdotq_s32(vdupq_n_s32(0), v0_1lf, v1_1l),
                        wsp_ggml_vdotq_s32(vdupq_n_s32(0), v0_1hf, v1_1h))), WSP_GGML_CPU_FP16_TO_FP32(x1->d)*WSP_GGML_CPU_FP16_TO_FP32(y1->d));
    }
...
void wsp_ggml_vec_dot_q5_1_q8_1(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (; ib + 1 < nb; ib += 2) {
        const block_q5_1 * WSP_GGML_RESTRICT x0 = &x[ib];
        const block_q5_1 * WSP_GGML_RESTRICT x1 = &x[ib + 1];
        const block_q8_1 * WSP_GGML_RESTRICT y0 = &y[ib];
        const block_q8_1 * WSP_GGML_RESTRICT y1 = &y[ib + 1];

        const uint8x16_t m4b = vdupq_n_u8(0x0F);

        summs0 += WSP_GGML_CPU_FP16_TO_FP32(x0->m) * WSP_GGML_CPU_FP16_TO_FP32(y0->s);
        summs1 += WSP_GGML_CPU_FP16_TO_FP32(x1->m) * WSP_GGML_CPU_FP16_TO_FP32(y1->s);
...
void wsp_ggml_vec_dot_q8_0_q8_0(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
...
#if defined(__ARM_FEATURE_MATMUL_INT8)
    if (nrc == 2) {
        const block_q8_0 * WSP_GGML_RESTRICT vx0 = vx;
        const block_q8_0 * WSP_GGML_RESTRICT vx1 = (const block_q8_0 *) ((const uint8_t*)vx + bx);
        const block_q8_0 * WSP_GGML_RESTRICT vy0 = vy;
        const block_q8_0 * WSP_GGML_RESTRICT vy1 = (const block_q8_0 *) ((const uint8_t*)vy + by);
        ...
        for (int i = 0; i < nb; i++) {
            const block_q8_0 * WSP_GGML_RESTRICT b_x0 = &vx0[i];
            const block_q8_0 * WSP_GGML_RESTRICT b_y0 = &vy0[i];

            const block_q8_0 * WSP_GGML_RESTRICT b_x1 = &vx1[i];
            const block_q8_0 * WSP_GGML_RESTRICT b_y1 = &vy1[i];
        ...
        case 128:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    const block_q8_0 * WSP_GGML_RESTRICT x0 = &x[ib + 0];
                    const block_q8_0 * WSP_GGML_RESTRICT x1 = &x[ib + 1];
                    const block_q8_0 * WSP_GGML_RESTRICT y0 = &y[ib + 0];
                    const block_q8_0 * WSP_GGML_RESTRICT y1 = &y[ib + 1];
                    ...
                    sumv0 = svmla_n_f32_x(pl16, sumv0, svcvt_f32_s32_x(pl16, svadd_x(pl16,
                                    ...
                                    svdot_s32(svdup_n_s32(0), qx0_1, qy0_1))), WSP_GGML_CPU_FP16_TO_FP32(x0->d)*WSP_GGML_CPU_FP16_TO_FP32(y0->d));
                    sumv1 = svmla_n_f32_x(pl16, sumv1, svcvt_f32_s32_x(pl16, svadd_x(pl16,
                                    svdot_s32(svdup_n_s32(0), qx1_0, qy1_0),
                                    svdot_s32(svdup_n_s32(0), qx1_1, qy1_1))), WSP_GGML_CPU_FP16_TO_FP32(x1->d)*WSP_GGML_CPU_FP16_TO_FP32(y1->d));
                }
        ...
        case 256:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    ...
                    sumv0 = svmla_n_f32_x(svptrue_b32(), sumv0, svcvt_f32_s32_x(svptrue_b32(),
                                svdot_s32(svdup_n_s32(0), qx0, qy0)), WSP_GGML_CPU_FP16_TO_FP32(x0->d)*WSP_GGML_CPU_FP16_TO_FP32(y0->d));
                    sumv1 = svmla_n_f32_x(svptrue_b32(), sumv1, svcvt_f32_s32_x(svptrue_b32(),
                                svdot_s32(svdup_n_s32(0), qx1, qy1)), WSP_GGML_CPU_FP16_TO_FP32(x1->d)*WSP_GGML_CPU_FP16_TO_FP32(y1->d));
                }
        ...
        case 512:
            {
                ...
                for (; ib + 1 < nb; ib += 2) {
                    ...
                    // scale creation
                    const float32_t deq1 = WSP_GGML_CPU_FP16_TO_FP32(x0->d)*WSP_GGML_CPU_FP16_TO_FP32(y0->d);
                    const float32_t deq2 = WSP_GGML_CPU_FP16_TO_FP32(x1->d)*WSP_GGML_CPU_FP16_TO_FP32(y1->d);
    ...
    for (; ib + 1 < nb; ib += 2) {
        ...
        sumv0 = vmlaq_n_f32(sumv0, vcvtq_f32_s32(vaddq_s32(
                        ...
                        wsp_ggml_vdotq_s32(vdupq_n_s32(0), x0_1, y0_1))), WSP_GGML_CPU_FP16_TO_FP32(x0->d)*WSP_GGML_CPU_FP16_TO_FP32(y0->d));

        sumv1 = vmlaq_n_f32(sumv1, vcvtq_f32_s32(vaddq_s32(
                        wsp_ggml_vdotq_s32(vdupq_n_s32(0), x1_0, y1_0),
                        wsp_ggml_vdotq_s32(vdupq_n_s32(0), x1_1, y1_1))), WSP_GGML_CPU_FP16_TO_FP32(x1->d)*WSP_GGML_CPU_FP16_TO_FP32(y1->d));
    }
    ...
    switch (vector_length) {
        case 128:
            for (int i = 0; i < nb; ++i) {
                ...
                const uint8_t * WSP_GGML_RESTRICT q2 = x[i].qs;
                const int8_t  * WSP_GGML_RESTRICT q8_sv = y[i].qs;
...
void wsp_ggml_vec_dot_q3_K_q8_K(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * WSP_GGML_RESTRICT q3_sv = x[i].qs;
        const uint8_t * WSP_GGML_RESTRICT qh_sv = x[i].hmask;
        const int8_t  * WSP_GGML_RESTRICT q8_sv = y[i].qs;
    ...
    *s = sum;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * WSP_GGML_RESTRICT qh = x[i].hmask;
        const int8_t  * WSP_GGML_RESTRICT q8 = y[i].qs;
...
#pragma GCC unroll 1
    ...
    if (nrc == 2) {
        ...
        const block_q8_K * WSP_GGML_RESTRICT y0 = y;
        const block_q8_K * WSP_GGML_RESTRICT y1 = (const block_q8_K *) ((const uint8_t *)vy + by);
        ...
        for (int i = 0; i < nb; ++i, ++x0, ++x1, ++y0, ++y1) {
            const uint8_t * WSP_GGML_RESTRICT qx0 = x0->qs;
            const uint8_t * WSP_GGML_RESTRICT qx1 = x1->qs;
            const  int8_t * WSP_GGML_RESTRICT qy0 = y0->qs;
            const  int8_t * WSP_GGML_RESTRICT qy1 = y1->qs;
            ...
            // adjust bias, apply superblock scale
            {
                ...
                const float32x4_t dmins = {
                    WSP_GGML_CPU_FP16_TO_FP32(x0->dmin) * y0->d,
                    WSP_GGML_CPU_FP16_TO_FP32(x0->dmin) * y1->d,
                    WSP_GGML_CPU_FP16_TO_FP32(x1->dmin) * y0->d,
                    WSP_GGML_CPU_FP16_TO_FP32(x1->dmin) * y1->d,
                };
                vfsum = vmlsq_f32(vfsum, vcvtq_f32_s32(vld1q_s32(bias)), dmins);

                const float32x4_t superblock_scale = {
                    WSP_GGML_CPU_FP16_TO_FP32(x0->d) * y0->d,
                    WSP_GGML_CPU_FP16_TO_FP32(x0->d) * y1->d,
                    WSP_GGML_CPU_FP16_TO_FP32(x1->d) * y0->d,
                    WSP_GGML_CPU_FP16_TO_FP32(x1->d) * y1->d,
                };
...
void wsp_ggml_vec_dot_q5_K_q8_K(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy,  size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * WSP_GGML_RESTRICT q5 = x[i].qs;
...
void wsp_ggml_vec_dot_q6_K_q8_K(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    if (nrc == 2) {
        ...
        for (int i = 0; i < nb; ++i, ++x0, ++x1, ++y0, ++y1) {
            const uint8_t * WSP_GGML_RESTRICT ql0 = x0->ql;
            const uint8_t * WSP_GGML_RESTRICT ql1 = x1->ql;
            const uint8_t * WSP_GGML_RESTRICT qh0 = x0->qh;
            const uint8_t * WSP_GGML_RESTRICT qh1 = x1->qh;
            const  int8_t * WSP_GGML_RESTRICT qy0 = y0->qs;
            const  int8_t * WSP_GGML_RESTRICT qy1 = y1->qs;
    ...
    for (int i = 0; i < nb; ++i) {
        const float d_all = WSP_GGML_CPU_FP16_TO_FP32(x[i].d);

        const uint8_t * WSP_GGML_RESTRICT q6 = x[i].ql;
        const uint8_t * WSP_GGML_RESTRICT qh = x[i].qh;
        const int8_t  * WSP_GGML_RESTRICT q8 = y[i].qs;

        const int8_t * WSP_GGML_RESTRICT scale = x[i].scales;
...
void wsp_ggml_vec_dot_iq2_s_q8_K(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
   ...
   static const uint8_t k_mask1[32] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * WSP_GGML_RESTRICT qh = x[i].qh;
        const uint16_t * WSP_GGML_RESTRICT signs = (const uint16_t *)(x[i].qs + QK_K/8);
        const int8_t  * WSP_GGML_RESTRICT q8 = y[i].qs;
...
void wsp_ggml_vec_dot_iq3_s_q8_K(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
   ...
   static const uint8_t k_mask1[32] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * WSP_GGML_RESTRICT qs = x[i].qs;
        const uint8_t * WSP_GGML_RESTRICT qh = x[i].qh;
        const uint16_t * WSP_GGML_RESTRICT signs = (const uint16_t *)x[i].signs;
        const int8_t   * WSP_GGML_RESTRICT q8 = y[i].qs;
...
void wsp_ggml_vec_dot_iq1_s_q8_K(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        sumf += y[i].d * WSP_GGML_CPU_FP16_TO_FP32(x[i].d) * (sumi1 + sumi2 + IQ1S_DELTA * sumi3);
    }
...
void wsp_ggml_vec_dot_iq1_m_q8_K(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        sumf += y[i].d * WSP_GGML_CPU_FP16_TO_FP32(scale.f16) * (vaddvq_s32(sumi1) + IQ1M_DELTA * vaddvq_s32(sumi2));
    }
...
void wsp_ggml_vec_dot_iq4_nl_q8_0(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (; ib + 1 < nb; ib += 2) {
        ...
        sumf +=
            WSP_GGML_CPU_FP16_TO_FP32(x[ib+0].d) * WSP_GGML_CPU_FP16_TO_FP32(y[ib + 0].d) * vaddvq_s32(prod_1) +
            WSP_GGML_CPU_FP16_TO_FP32(x[ib+1].d) * WSP_GGML_CPU_FP16_TO_FP32(y[ib + 1].d) * vaddvq_s32(prod_2);
    }
...
void wsp_ggml_vec_dot_iq4_xs_q8_K(int n, float * WSP_GGML_RESTRICT s, size_t bs, const void * WSP_GGML_RESTRICT vx, size_t bx, const void * WSP_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int ibl = 0; ibl < nb; ++ibl) {
        ...
        sumf += WSP_GGML_CPU_FP16_TO_FP32(x[ibl].d) * y[ibl].d * (sumi1 + sumi2);
    }
