...
void wsp_ggml_vec_dot_f16(int n, float * WSP_GGML_RESTRICT s, size_t bs, wsp_ggml_fp16_t * WSP_GGML_RESTRICT x, size_t bx, wsp_ggml_fp16_t * WSP_GGML_RESTRICT y, size_t by, int nrc) {
    ...
    wsp_ggml_float sumf = 0.0;
...
#if defined(WSP_GGML_SIMD)
    #if defined(__ARM_FEATURE_SVE)
        const int wsp_ggml_f16_epr = svcnth();
        const int wsp_ggml_f16_step = 8 * wsp_ggml_f16_epr;
        const int np = n - (n % wsp_ggml_f16_step);
        const int np2 = n - (n % wsp_ggml_f16_epr);
        ...
        for (int i = 0; i < np; i += wsp_ggml_f16_step) {
            wsp_ggml_sve_f16_fma_widened(&sum1_lo, &sum1_hi, WSP_GGML_F16x_VEC_LOAD(x + i + 0 * wsp_ggml_f16_epr, 0), WSP_GGML_F16x_VEC_LOAD(y + i + 0 * wsp_ggml_f16_epr, 0));
            wsp_ggml_sve_f16_fma_widened(&sum2_lo, &sum2_hi, WSP_GGML_F16x_VEC_LOAD(x + i + 1 * wsp_ggml_f16_epr, 1), WSP_GGML_F16x_VEC_LOAD(y + i + 1 * wsp_ggml_f16_epr, 1));
            wsp_ggml_sve_f16_fma_widened(&sum3_lo, &sum3_hi, WSP_GGML_F16x_VEC_LOAD(x + i + 2 * wsp_ggml_f16_epr, 2), WSP_GGML_F16x_VEC_LOAD(y + i + 2 * wsp_ggml_f16_epr, 2));
            wsp_ggml_sve_f16_fma_widened(&sum4_lo, &sum4_hi, WSP_GGML_F16x_VEC_LOAD(x + i + 3 * wsp_ggml_f16_epr, 3), WSP_GGML_F16x_VEC_LOAD(y + i + 3 * wsp_ggml_f16_epr, 3));
            wsp_ggml_sve_f16_fma_widened(&sum1_lo, &sum1_hi, WSP_GGML_F16x_VEC_LOAD(x + i + 4 * wsp_ggml_f16_epr, 4), WSP_GGML_F16x_VEC_LOAD(y + i + 4 * wsp_ggml_f16_epr, 4));
            wsp_ggml_sve_f16_fma_widened(&sum2_lo, &sum2_hi, WSP_GGML_F16x_VEC_LOAD(x + i + 5 * wsp_ggml_f16_epr, 5), WSP_GGML_F16x_VEC_LOAD(y + i + 5 * wsp_ggml_f16_epr, 5));
            wsp_ggml_sve_f16_fma_widened(&sum3_lo, &sum3_hi, WSP_GGML_F16x_VEC_LOAD(x + i + 6 * wsp_ggml_f16_epr, 6), WSP_GGML_F16x_VEC_LOAD(y + i + 6 * wsp_ggml_f16_epr, 6));
            wsp_ggml_sve_f16_fma_widened(&sum4_lo, &sum4_hi, WSP_GGML_F16x_VEC_LOAD(x + i + 7 * wsp_ggml_f16_epr, 7), WSP_GGML_F16x_VEC_LOAD(y + i + 7 * wsp_ggml_f16_epr, 7));
        }

        for (int i = np; i < np2; i += wsp_ggml_f16_epr) {
            wsp_ggml_sve_f16_fma_widened(&sum1_lo, &sum1_hi, WSP_GGML_F16x_VEC_LOAD(x + i, 0), WSP_GGML_F16x_VEC_LOAD(y + i, 0));
        }
        ...
        if (np2 < n) {
            ...
            wsp_ggml_sve_f16_fma_widened(&sum1_lo, &sum1_hi, rx, ry);
        }
        ...
        sumf = wsp_ggml_sve_sum_f32x2(sum1_lo, sum1_hi);
    #elif defined(__riscv_v_intrinsic)
