...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_pool_1d(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op, wsp_ggml_op_pool op_pool) {
    ...
    snprintf(base, sizeof(base), "kernel_pool_1d_%s_%s", pool_str, wsp_ggml_type_name(op->src[0]->type));
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_set_rows(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    char base[256];
    char name[256];
    ...
    snprintf(base, 256, "kernel_set_rows_%s_%s_%s", wsp_ggml_type_name(tsrc), wsp_ggml_type_name(tidx), wsp_ggml_type_name(tdst));
    snprintf(name, 256, "%s", base);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_diag(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    char base[256];
    char name[256];

    const int n = op->src[0]->ne[0];

    snprintf(base, 256, "kernel_diag_%s", wsp_ggml_type_name(op->src[0]->type));
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_concat(wsp_ggml_metal_library_t lib, wsp_ggml_type tsrc) {
    char base[256];
    char name[256];

    snprintf(base, 256, "kernel_concat_%s", wsp_ggml_type_name(tsrc));
    snprintf(name, 256, "%s", base);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_lightning_indexer(
    ...
    snprintf(name, 256, "kernel_lightning_indexer_%s", wsp_ggml_type_name(op->src[1]->type));

    wsp_ggml_metal_pipeline_with_params res = wsp_ggml_metal_library_get_pipeline(lib, name);
    if (!res.pipeline) {
        res = wsp_ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
    }

    return res;
}

wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_dsv4_hc(wsp_ggml_metal_library_t lib, wsp_ggml_op op) {
    const char * name = nullptr;

    switch (op) {
        case WSP_GGML_OP_DSV4_HC_COMB: name = "kernel_dsv4_hc_comb_f32"; break;
        case WSP_GGML_OP_DSV4_HC_PRE:  name = "kernel_dsv4_hc_pre_f32";  break;
        case WSP_GGML_OP_DSV4_HC_POST: name = "kernel_dsv4_hc_post_f32"; break;
        default: WSP_GGML_ABORT("fatal error");
    }
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_gated_delta_net(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    ...
    const int K = wsp_ggml_get_op_params_i32(op, 0);
    ...
    if (!res.pipeline) {
        ...
        wsp_ggml_metal_cv_set_int16(cv, K,    FC_GATED_DELTA_NET + 2);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_solve_tri(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    char base[256];
    char name[256];
    ...
    snprintf(base, 256, "kernel_solve_tri_%s", wsp_ggml_type_name(op->src[0]->type));
    ...
    if (!res.pipeline) {
        ...
        wsp_ggml_metal_cv_set_int16(cv, nsg, FC_SOLVE_TRI + 0);
        wsp_ggml_metal_cv_set_int16(cv, n,   FC_SOLVE_TRI + 1);
        wsp_ggml_metal_cv_set_int16(cv, k,   FC_SOLVE_TRI + 2);
    ...
    res.nsg  = nsg;
    res.smem = WSP_GGML_PAD(WSP_GGML_PAD(n, 32)*nsg*sizeof(float), 16);

    return res;
}

wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_mul_mv_ext(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op, int nsg, int nxpsg, int r1ptg) {
    char base[256];
    char name[256];
    ...
    WSP_GGML_ASSERT(ne12 <= INT16_MAX && r2 <= INT16_MAX && r3 <= INT16_MAX);
    ...
    if (!res.pipeline) {
        ...
        wsp_ggml_metal_cv_set_int16(cv, (int16_t) ne12, FC_MUL_MV + 2);
        wsp_ggml_metal_cv_set_int16(cv, (int16_t) r2,   FC_MUL_MV + 3);
        wsp_ggml_metal_cv_set_int16(cv, (int16_t) r3,   FC_MUL_MV + 4);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_mul_mm(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    ...
    WSP_GGML_ASSERT(op->src[1]->ne[2] <= INT16_MAX && op->src[1]->ne[3] <= INT16_MAX);
    ...
    if (!res.pipeline) {
        ...
        wsp_ggml_metal_cv_set_int16(cv, ne12,  FC_MUL_MM + 2);
        wsp_ggml_metal_cv_set_int16(cv, ne13,  FC_MUL_MM + 3);
        wsp_ggml_metal_cv_set_int16(cv, r2,    FC_MUL_MM + 4);
        wsp_ggml_metal_cv_set_int16(cv, r3,    FC_MUL_MM + 5);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_mul_mv(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    ...
    WSP_GGML_ASSERT(ne12 <= INT16_MAX && ne13 <= INT16_MAX);
    ...
    if (!res.pipeline) {
        ...
        wsp_ggml_metal_cv_set_int16(cv, nsg,            FC_MUL_MV + 0);
        wsp_ggml_metal_cv_set_int16(cv, (int16_t) ne12, FC_MUL_MV + 2);
        wsp_ggml_metal_cv_set_int16(cv, r2,             FC_MUL_MV + 3);
        wsp_ggml_metal_cv_set_int16(cv, r3,             FC_MUL_MV + 4);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_mul_mv_id(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    ...
    if (!res.pipeline) {
        ...
        wsp_ggml_metal_cv_set_int16(cv, nsg, FC_MUL_MV + 0);
        wsp_ggml_metal_cv_set_int16(cv, 1,   FC_MUL_MV + 2);
        wsp_ggml_metal_cv_set_int16(cv, 1,   FC_MUL_MV + 3);
        wsp_ggml_metal_cv_set_int16(cv, 1,   FC_MUL_MV + 4);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_fwht(wsp_ggml_metal_library_t lib, int n) {
    char base[256];
    char name[256];
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_flash_attn_ext_kv_f16(
        wsp_ggml_metal_library_t lib,
        const wsp_ggml_tensor * op) {
    assert(op->op == WSP_GGML_OP_FLASH_ATTN_EXT);
    ...
    snprintf(base, 256, "kernel_flash_attn_ext_kv_%s_f16", wsp_ggml_type_name(op->src[1]->type));

    wsp_ggml_metal_pipeline_with_params res = wsp_ggml_metal_library_get_pipeline(lib, base);
    if (!res.pipeline) {
        res = wsp_ggml_metal_library_compile_pipeline(lib, base, base, nullptr);
    }
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_bin(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op, int32_t n_fuse) {
    char base[256];
    char name[256];
    ...
    switch (op->op) {
        case WSP_GGML_OP_ADD: op_num = 0; break;
        case WSP_GGML_OP_SUB: op_num = 1; break;
        case WSP_GGML_OP_MUL: op_num = 2; break;
        case WSP_GGML_OP_DIV: op_num = 3; break;
        default: WSP_GGML_ABORT("fatal error");
    };

    const char * t0_str = wsp_ggml_type_name(op->src[0]->type);
    const char * t1_str = wsp_ggml_type_name(op->src[1]->type);
    const char * t_str  = wsp_ggml_type_name(op->type);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_bin_one(wsp_ggml_metal_library_t lib, wsp_ggml_op op) {
    char base[256];
    char name[256];
    ...
    if (!res.pipeline) {
        ...
        wsp_ggml_metal_cv_set_int16(cv, op_num, FC_BIN + 0);
        wsp_ggml_metal_cv_set_int16(cv, 1,      FC_BIN + 1);
        wsp_ggml_metal_cv_set_bool (cv, false,  FC_BIN + 2);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_rope(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    assert(op->op == WSP_GGML_OP_ROPE || op->op == WSP_GGML_OP_ROPE_BACK);

    const bool is_back = op->op == WSP_GGML_OP_ROPE_BACK;
    ...
    if (!res.pipeline) {
        ...
        wsp_ggml_metal_cv_set_bool(cv, is_back,   FC_ROPE + 1);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_im2col(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    ...
    if (KH*KW <= 1024) {
        ...
        snprintf(base, 256, "kernel_im2col_ext_%s", wsp_ggml_type_name(op->type));
    }
    snprintf(name, 256, "%s", base);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_col2im_1d(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    assert(op->op == WSP_GGML_OP_COL2IM_1D);

    WSP_GGML_ASSERT(wsp_ggml_is_contiguous(op->src[0]));
    WSP_GGML_ASSERT(op->src[0]->type == WSP_GGML_TYPE_F32 || op->src[0]->type == WSP_GGML_TYPE_F16 || op->src[0]->type == WSP_GGML_TYPE_BF16);

    char base[256];
    char name[256];

    snprintf(base, 256, "kernel_col2im_1d_%s", wsp_ggml_type_name(op->src[0]->type));
    snprintf(name, 256, "%s", base);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_snake(wsp_ggml_metal_library_t lib, enum wsp_ggml_type type) {
    WSP_GGML_ASSERT(type == WSP_GGML_TYPE_F32 || type == WSP_GGML_TYPE_F16 || type == WSP_GGML_TYPE_BF16);

    char base[256];
    char name[256];

    snprintf(base, 256, "kernel_snake_%s", wsp_ggml_type_name(type));
    snprintf(name, 256, "%s", base);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_conv_2d_dw(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op, bool tiled) {
    assert(op->op == WSP_GGML_OP_CONV_2D_DW);
    ...
    snprintf(base, 256, "kernel_conv_2d_dw%s_%s_%s",
             tiled ? "_tiled" : "",
             wsp_ggml_type_name(op->src[0]->type), wsp_ggml_type_name(op->src[1]->type));
    snprintf(name, 256, "%s", base);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_pad(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    ...
    snprintf(base, 256, "kernel_pad_%s%s", wsp_ggml_type_name(op->src[0]->type), is_c4 ? "_4" : "");
    snprintf(name, 256, "%s", base);
...
wsp_ggml_metal_pipeline_with_params wsp_ggml_metal_library_get_pipeline_silu_back(wsp_ggml_metal_library_t lib, const wsp_ggml_tensor * op) {
    assert(op->op == WSP_GGML_OP_SILU_BACK);

    char base[256];
    char name[256];

    snprintf(base, 256, "kernel_silu_back_%s", wsp_ggml_type_name(op->src[0]->type));
    snprintf(name, 256, "%s", base);
