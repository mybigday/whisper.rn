...
import type { WhisperContext, WhisperVadContext, VadOptions } from '../../../src'
...
interface InternalRealtimeOptions {
  ...
  vadMode: 'continuous' | 'push-to-talk' | 'smart'
}
...
export class RealtimeTranscriber {
  ...
  private concurrentTranscriptions = 0
  ...
  constructor(
    ...
    this.options = {
      ...
      concurrentTranscriptions: options.concurrentTranscriptions || 1,
      vadMode: options.vadMode || 'smart',
    }

    // Apply VAD preset if specified
    if (this.options.vadPreset && VAD_PRESETS[this.options.vadPreset]) {
      this.options.vadOptions = { ...VAD_PRESETS[this.options.vadPreset], ...this.options.vadOptions }
    }
  ...
  private async processSliceVAD(slice: any): Promise<void> {
    try {
      ...
      if (isSpeech) {
        ...
        if (vadEvent.timestamp !== this.lastSpeechDetectedTime) {
          this.lastSpeechDetectedTime = vadEvent.timestamp
          ...
          this.log(`Skipping transcription for slice ${slice.index} - same detection time as last`)
        }
      } else {
  ...
  private async queueSliceForTranscription(slice: any): Promise<void> {
    try {
      ...
      if (this.concurrentTranscriptions < this.options.concurrentTranscriptions) {
   ...
   */
  private async detectSpeech(audioData: Buffer, sliceIndex: number): Promise<VADEvent> {
    if (!this.vadContext) {
    ...
    try {
      ...
      const vadSegments = await this.vadContext.detectSpeechData(audioBase64, this.options.vadOptions)
  ...
  private async processTranscriptionQueue(): Promise<void> {
    while (
      ...
      this.concurrentTranscriptions < this.options.concurrentTranscriptions
    ) {
      const item = this.transcriptionQueue.shift()
      if (item) {
        this.processTranscription(item).catch(error => {
   ...
   */
  private async processTranscription(item: { sliceIndex: number; audioData: Buffer }): Promise<void> {
    if (!this.isActive) {
      return
    }

    this.concurrentTranscriptions += 1
    ...
    try {
      const { promise } = this.whisperContext.transcribeData(item.audioData.toString('base64'), {
      ...
      if (this.transcriptionQueue.length > 0) {
        await this.processTranscriptionQueue()
      } else if (this.concurrentTranscriptions === 0) {
  ...
  /**
   * Process any remaining items in the queue when stopping
   */
  private async processRemainingQueue(): Promise<void> {
    // Wait for current transcriptions to finish using recursive promise
    const waitForConcurrentToFinish = async (maxWaitTime = 10000): Promise<void> => {
      const startTime = Date.now()

      const checkAndWait = async (): Promise<void> => {
        if (this.concurrentTranscriptions === 0 || (Date.now() - startTime) >= maxWaitTime) {
          return
        }
    ...
    await waitForConcurrentToFinish()

    // Process remaining queue items in batches
    const remainingItems = [...this.transcriptionQueue]
    this.transcriptionQueue = []

    if (remainingItems.length > 0) {
      const batchSize = Math.min(remainingItems.length, this.options.concurrentTranscriptions)

      const processBatch = async (startIndex: number): Promise<void> => {
        if (startIndex >= remainingItems.length) {
          return
        }

        const batch = remainingItems.slice(startIndex, startIndex + batchSize)
        await Promise.all(batch.map(item => this.processTranscription(item)))
  ...
  getStatistics() {
    return {
      ...
      concurrentTranscriptions: this.concurrentTranscriptions,
      audioStats: {
  ...
  reset(): void {
    ...
    this.transcriptionQueue = []
    this.concurrentTranscriptions = 0
    this.isTranscribing = false
    this.accumulatedData = new Uint8Array(0)
